...
import numpy as np
import torch
import HomeSmartGridEnv


def ReplayBuffer(state_dim, is_atari, atari_preprocessing, batch_size, buffer_size, alpha, beta, device):
    if is_atari:
        return AtariBuffer(state_dim, atari_preprocessing, batch_size, buffer_size, device)
    else:
        return StandardBuffer(state_dim, batch_size, buffer_size, alpha, beta, device)
...
class StandardBuffer(object):
    def __init__(self, state_dim, batch_size, buffer_size, alpha, beta, device):
        self.batch_size = batch_size
        self.max_size = int(buffer_size)
        self.device = device
        ...
        self.beta = beta
        self.alpha = alpha  # 优先级的重要性指数

        self.priorities = np.zeros((self.max_size,), dtype=np.float32)  # 优先级数组

    def add(self, state, action, next_state, reward, done, episode_done, episode_start):
        if self.crt_size == self.max_size:
            min_priority_idx = np.argmin(self.priorities[:self.crt_size])
            if self.priorities[self.ptr] > self.priorities[min_priority_idx]:
                self.ptr = min_priority_idx
        ...
        td_error = abs(reward)  # 使用 reward 作为替代值
        # 将新经验的优先级设置为最高
        self.priorities[self.ptr] = max((abs(reward) + 1e-6) ** self.alpha, 1e-6)
    ...
    def exp_sample(self):
        ...
        # 计算采样概率
        total_priority = max(self.priorities[:self.crt_size].sum(),1e-6)

        probs = np.clip(self.priorities[:self.crt_size] / total_priority, 1e-6, None)


        # 根据概率采样
        indices = np.random.choice(self.crt_size, self.batch_size, p=probs,replace=True)

        # 计算权重
        weights = np.power(self.crt_size * probs[indices], -self.beta)
        ...
        return (
            ...
            indices,  # 返回采样索引以便以后更新优先级
			torch.FloatTensor(weights).to(self.device) # 返回计算得到的权重
        )
    def sample(self):
        if self.crt_size == 0:
            raise ValueError("The replay buffer is empty!")
    ...
    def update_priorities(self, indices, td_errors):
        self.priorities[indices] = np.clip((td_errors + 1e-6) ** self.alpha, 1e-6, None)
...
def make_env(env_name, atari_preprocessing):
    if env_name == "HomeSmartGridEnv":
        env = HomeSmartGridEnv.MyEnv()
        is_atari = False
